...
export async function email(message, env, ctx) {
	...
	try {
		...
		//转发到TG
			...
			// Per-address forwarding map: if mapping exists for current recipient, use it and skip global forwarding
			if (forwardStatus === settingConst.forwardStatus.OPEN && forwardMap) {
				try {
					const toAddr = (message.to || '').trim().toLowerCase();
					const mapping = typeof forwardMap === 'object' ? forwardMap : {};
					const targets = Array.isArray(mapping[toAddr]) ? mapping[toAddr] : [];
					if (targets.length > 0) {
						await Promise.all(targets.map(async (addr) => {
							try { await message.forward(addr); } catch (e) { console.error('Forward map failed:', addr, e); }
						}));
						return;
					}
				} catch (e) {
					console.error('Forward map error:', e);
				}
			}
