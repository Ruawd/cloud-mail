...
## 学校导航页（新增）
- 登录后默认跳转到独立的“学校导航”页面 `/school`
- 顶部展示：校徽 + 标题（右侧） + 三个主入口卡片：进入邮箱、学校官网、学生证生成
- 背景图全宽铺满（无灰边），叠加渐变遮罩；卡片采用 App 风格，悬停时上浮并带阴影

## v2.4.0 更新
- 邮件发送正文中的图片统一转换为内嵌附件，兼容更多客户端
- 支持设置“注册邮箱前缀最小位数”，注册/绑定/添加邮箱都会校验
- 登录页新增 LinuxDo OAuth2，一键同步头像、等级并绑定邮箱
- 修复移动端“全部邮件”工具栏图标重叠的问题
- 修复 TG Bot 自定义域名前带 https:// 时无法查看邮件的 Bug
